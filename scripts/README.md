...
| `seed.sh [api url]` | Push `docs/data/test-data.json` (accounts, vehicles, records) into a running API through its public endpoints; idempotent; verifies accounts when the API runs in Development (slice S38) |
...
`lib/common.sh` holds the shared helpers (toolchain loading, starting and stopping the apps, readiness waits) and is sourced, never run. `lib/serve-web.mjs` is a dependency-free static server with SPA fallback and an `/api` proxy, the same shape as the Nginx container. `lib/seed.mjs` is the seeder behind `seed.sh`.
